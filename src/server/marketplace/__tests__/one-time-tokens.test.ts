...
describe("tokens de un solo uso", () => {
  ...
  /*
    El contador del límite por email. Cuenta EMISIONES desde `since`, y por eso
    la ventana se compara contra `issuedAt`: con la caducidad, un token de hace
    45 minutos —que expiró hace 15— seguía contando dentro de una ventana de 30
    minutos, porque su `expiresAt` caía dentro. Media hora de más sobre la
    ventana, o sea un límite que rechaza a quien ya debería poder pedir otro.
  */
  it("cuenta las emisiones por su fecha de emisión y no por su caducidad", async () => {
    const store = setup();
    const email = "gerencia@hospital.example";
    const media_hora = 30 * 60;

    // Tres emisiones de hace 45 minutos: ya caducadas y fuera de la ventana.
    for (let index = 0; index < 3; index += 1) {
      await issueOneTimeToken(
        { subscriptionId: "sub_1", email },
        { store, pepper: PEPPER, now: NOW - 45 * 60 },
      );
    }
